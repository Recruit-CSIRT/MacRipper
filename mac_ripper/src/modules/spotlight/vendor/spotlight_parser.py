...
class FileMetaDataListing:
    ...
    def textWriter(self, file):
        try:
            ...
            info_list = [
                dashed_line,
                "Inode_Num{}{}".format(sep, self.id),
                "Flags{}{}".format(sep, self.flags),
                "Store_ID{}{}".format(sep, self.item_id),
                "Parent_Inode_Num{}{}".format(sep, self.parent_id),
                "Last_Updated{}{}".format(sep, self.parent_id, self.ConvertEpochToUtcDateStr(self.date_updated))
            ]

            for info in info_list:
                cnt = info + new_line
                file.write(bytes(cnt.encode('utf8')))

            for k, v in sorted(self.meta_data_dict.items()):
                orig_debug = v
                v = self.StringifyValue(v)
                cnt = k + sep + v + new_line
                file.write(bytes(cnt.encode('utf8')))
        except Exception as ex:
            log.exception("Exception trying to print data : ")

    def jsonWriter(self, file):
        data = {}
        data['Inode_Num'] = self.id
        data['Flags'] = self.flags
        data['Store_ID'] = self.item_id
        data['Parent_Inode_Num'] = self.parent_id
        data['Last_Updated'] = self.ConvertEpochToUtcDateStr(self.date_updated).strftime("%m/%d/%Y, %H:%M:%S")
        try:
            for k, v in sorted(self.meta_data_dict.items()):
                orig_debug = v
                v = self.StringifyValue(v)
    ...
    def Print(self, file):
        self.textWriter(file)
        #self.jsonWriter(file)
...
def ProcessStoreDb(input_file_path, output_folder, file_name_prefix='store'):
